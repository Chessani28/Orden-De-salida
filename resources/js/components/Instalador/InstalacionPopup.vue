                ...
                <div v-if="instalacion && typeof instalacion.img === 'string'">
                    ...
                    <ul class="image-list">
                        <li v-for="(image, index) in instalacion.img.split(',')" :key="index">
                            ...
                            <i class="ni ni-fat-remove text-dark" @click="eliminarImagen(index)"></i>
                        </li>
                    </ul>
                </div>
            </div>
            <p>ID de programación: {{ instalacion.id_programacion }}</p>
            <form @submit.prevent="guardarImagenes">
                <input type="file" multiple @change="cargarImagenes" />
                <button type="submit" class="submit-button">Guardar Imágenes</button>
            </form>
        </div>
...
<style scoped>
@import 'css/instalacionPopup.css'
</style>
